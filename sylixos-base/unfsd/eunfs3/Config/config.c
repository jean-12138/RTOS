 ...
 */
static void set_ipnet(char *addr)
{
    char *pos, *net;
    int num = 1;
...
int exports_parse(void)
{
    ...
    dict = iniparser_load(opt_exports);
    if (!dict) {
#endif
        logmsg(LOG_CRIT, "could not open '%s', use default config",
               opt_exports);

        set_ipnet(UNFSD_CFG_DEF_IPNET0);
        add_option(UNFSD_CFG_DEF_OPT);
        add_item(UNFSD_CFG_DEF_ITEM);
        ...
        for (i = 0; i < nsec; i++) {
            ...
            /*
             * ipnet=127.0.0.1/255.255.255.0
             */
            snprintf(key, sizeof(key), "%s:ipnet", secname);
            strncpy(buf, iniparser_getstring(dict, key, UNFSD_CFG_DEF_IPNET0), sizeof(buf));
            set_ipnet(buf);

            /*
             * opt=rw,root_squash
             */
            snprintf(key, sizeof(key), "%s:opt", secname);
            strncpy(buf, iniparser_getstring(dict, key, UNFSD_CFG_DEF_OPT), sizeof(buf));
            p = buf;
            while((opt = strtok_r(p, ",", &safe_ptr)) != NULL) {
                add_option(opt);
                p = NULL;
            }
             ...
             * password=xxxx
             */
            snprintf(key, sizeof(key), "%s:password", secname);
            strncpy(buf, iniparser_getstring(dict, key, ""), sizeof(buf));
            if (strlen(buf)) {
                add_option_with_value("password", buf);
            }

            /*
             * anonuid=xxxx
             */
            snprintf(key, sizeof(key), "%s:anonuid", secname);
            strncpy(buf, iniparser_getstring(dict, key, ""), sizeof(buf));
            if (strlen(buf)) {
                add_option_with_value("anonuid", buf);
            }

            /*
             * anonuid=xxxx
             */
            snprintf(key, sizeof(key), "%s:anongid", secname);
            strncpy(buf, iniparser_getstring(dict, key, ""), sizeof(buf));
            if (strlen(buf)) {
                add_option_with_value("anongid", buf);
            }
